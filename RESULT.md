...
| 항목 | 보완 내용 |
|---|---|
| 서버측 채팅 입력 검증 | `POST /api/chat`에 `message` 1~2000자 제한, 제어문자 제거+공백 거부, `conversation_id`는 `[A-Za-z0-9_-]+` 형식만 허용하도록 Pydantic validator 추가. 2000자 초과, 공백 메시지, `../etc/passwd` 형태의 `conversation_id` 모두 422로 정상 차단되는 것을 배포 환경에서 확인 |
| 요약/통계 API 기간 필터 | `GET /api/data/summary`, `GET /api/data/statistics`에 선택적 쿼리 파라미터 `start`/`end`(`YYYY-MM-DD`) 추가. 지정 시 해당 기간만 집계, 생략 시 기존과 동일하게 전체 기간 반환. `/api/chat`은 아직 이 파라미터와 연동하지 않아 항상 전체 기간 기준으로 답변(알려진 제한사항) |
